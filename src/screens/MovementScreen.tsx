...
const MovementScreen = () => {
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await fetch(process.env.EXPO_PUBLIC_API_IP + '/branches/options');
        if (!response.ok) {
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(process.env.EXPO_PUBLIC_API_IP + '/products/options');
        if (!response.ok) {
          throw new Error(`Erro: ${response.status}`);
        }
        const data = await response.json();
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(process.env.EXPO_PUBLIC_API_IP + '/movements', movementData, {
        headers: {
      ...
      if (response.status === 201) {
        ...
        Alert.alert('Erro', response.data.message || 'Falha ao cadastrar a movimentação.');
      }
    } catch (error) {
      console.error('Erro ao cadastrar movimentação:', error.response ? error.response.data : error.message);
